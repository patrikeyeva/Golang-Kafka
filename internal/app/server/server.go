...
import (
	...
	"errors"
	"fmt"
	"homework6/internal/pkg/repository"
	"io"
	"net/http"
	"strconv"
...
func CreateRouter(ctx context.Context, server Server) *mux.Router {
	...
	router.HandleFunc("/article", func(w http.ResponseWriter, req *http.Request) {
		...
		go func(req *http.Request, body []byte) {
			err := server.SendToKafka(req, body)
			if err != nil {
				fmt.Println("Error when trying to send a request to kafka", err)
			}
		}(req, body)

		switch req.Method {
		case http.MethodPost:
			server.Create(w, req, body)
		case http.MethodPut:
			server.Update(w, req, body)
		case http.MethodGet:
			server.Get(w, req)
	...
	router.HandleFunc("/comment", func(w http.ResponseWriter, req *http.Request) {
		...
		switch req.Method {
		case http.MethodPost:
			server.CreateComment(w, req, body)
		default:
			fmt.Println("error")
		}
	})
...
func (server *Server) SendToKafka(req *http.Request, reqBody []byte) error {
	message, err := NewServerMessage(req, reqBody)
	if err != nil {
		return err
	}
	...
	err = server.Sender.SendMessage(messageToKafka)
	if err != nil {
		return err
	}
	return nil
...
func (server *Server) Create(w http.ResponseWriter, req *http.Request, body []byte) {
...
func (server *Server) Update(w http.ResponseWriter, req *http.Request, body []byte) {
...
func (server *Server) CreateComment(w http.ResponseWriter, req *http.Request, body []byte) {
